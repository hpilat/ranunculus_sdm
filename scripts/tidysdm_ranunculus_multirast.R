...
set.seed(1234567)
ran_occ_thin <- thin_by_dist(ran_occ, dist_min = km2m(5))
nrow(ran_occ_thin)
...
ran_occ_thin <- ran_occ_thin %>% 
  bind_cols(terra::extract(predictors_multi_num, ran_occ_thin, ID = FALSE))
...
suggested_vars <- names(predictors_multi_num)

# inspect the variables for collinearity
pairs(predictors_multi_num[[suggested_vars]])
...
ran_occ_thin <- ran_occ_thin[[suggested_vars]] # suggested_vars part causes an error
predictors_uncorr <- filter_high_cor(predictors_multi_num, cutoff = 0.8, names = TRUE)
predictors_uncorr

# remove highly correlated predictors
ran_occ_thin <- ran_occ_thin %>% select(all_of(c(predictors_uncorr, "class")))
